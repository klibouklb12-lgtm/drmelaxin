...
Task ID: 6
Agent: Super Z (Principal Architect)
Task: Make the project Netlify-deployable via GitHub. Fix SQLite issue (won't work on serverless). Prepare clean repo for push. Honest answer on 500k visits/month.

Work Log:
- Full code scan revealed: /api/orders used Prisma/SQLite — WON'T WORK on Netlify (ephemeral filesystem)
- Found Google Apps Script already deployed + working (URL in .env)
- Found admin panel already uses Google Sheets (not Prisma) — no changes needed there
- Found Storefront already uses Google Sheets for stock — no changes needed there
- Only /api/orders route needed changing
- Rewrote src/lib/orders.ts:
  • Removed Prisma import entirely
  • createOrder() now posts to Google Sheets via fetch()
  • Order number generated by Apps Script (AUR-YYYY-NNNNNN)
  • In-memory rate limiting (per serverless instance, basic abuse prevention)
  • Server-side price re-derivation preserved (never trusts client)
  • Same function signature — /api/orders route unchanged
- Fixed next.config.ts: removed output: "standalone" (Netlify handles Next.js natively)
- Fixed package.json: build script is now just "next build" (removed standalone cp commands)
- Updated netlify.toml: npm run build, Node 20, documented env vars
- Created .env.example with all required vars documented
- Created download/DEPLOY.md: step-by-step Netlify deployment guide
- Tested orders endpoint locally:
  • Valid order (wilaya=1, commune=1): SUCCESS → AUR-2026-000001 in Google Sheet
  • Invalid phone: REJECTED with VALIDATION error
  • Server-side price re-derivation: confirmed (3900 DA, not client-sent)
- Cleaned git repo:
  • Removed .env, db/custom.db, forensics/, .zscripts/, examples/, tool-results/
  • Removed large PNG/zip/bundle files (122MB tar.gz was blocking GitHub push)
  • Updated .gitignore to prevent re-adding
  • Reset git history (old history had random UUID commits + sensitive files)
  • Fresh commit: 1.9MB repo (down from 179MB)
- No remote set yet — waiting for user's GitHub info

Honest answer on 500k visits/month:
- Netlify free tier = 100GB bandwidth/month
- With CDN + Service Worker:
  • 100% unique (ad traffic): 165GB → DOES NOT FIT
  • 50% repeat: 84GB → FITS
  • 70% repeat: 51GB → FITS
  • 90% repeat: 18GB → FITS
- Conclusion: 500k visits/month fits IF ~50%+ are repeat visitors
- For pure ad traffic, Cloudflare Pages (unlimited bandwidth) is safer
- Same code works on both — zero lock-in

Stage Summary:
- ✅ /api/orders now posts to Google Sheets (no Prisma, works on Netlify)
- ✅ Build compiles clean (npm run build)
- ✅ Orders tested end-to-end (AUR-2026-000001 created in Google Sheet)
- ✅ next.config.ts fixed (removed standalone)
- ✅ package.json fixed (build is just "next build")
- ✅ netlify.toml properly configured for Netlify
- ✅ .env.example created with all vars documented
- ✅ DEPLOY.md created with step-by-step guide
